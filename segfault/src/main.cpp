...
static void ERROR_SIGNAL_HANDLER_FUNC(int sig_nr, siginfo_t* info, void* ucontext)
{
	...
	{
		log("Failed printing signal err!\n");
	}
		...
		if (err == 0) {
			...
			if (err == 0) {
				...
				while (true) // unwind loop
				{
					...
					logf("%s +%p ", fallback_name ? fallback_name : demangled ? demangled
												  : ((func_name[0] == '\0') ? "?" : func_name),
					    (void*) offset);
